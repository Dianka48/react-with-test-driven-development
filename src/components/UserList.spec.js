...
describe("User List", () => {
  ...
  describe("Internationalization", () => {
    ...
    it("displays header and navigation links in slovak after selecting the language", async () => {
      setup();
      await screen.findByText("user4");
      userEvent.click(screen.getByTitle("Slovensky"));
      expect(screen.getByText(sk.users)).toBeInTheDocument();
      expect(screen.getByText(sk.nextPage)).toBeInTheDocument();
      expect(screen.getByText(sk.previousPage)).toBeInTheDocument();
    });
  });
});
