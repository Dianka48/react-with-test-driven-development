...
describe("Profile Card", () => {
  ...
  it("displays input for username after cliking edit", () => {
    setup();
    expect(
  ...
  it("sendds request to the endpoint with body having updated username", async () => {
    setupInEditMode();
  ...
  it("sends request with body having username even when user does not updated it", async () => {
    setupInEditMode();
    userEvent.click(saveButton);
    const spinner = screen.getByRole("status");
    await waitForElementToBeRemoved(spinner);
  ...
  it("updates username in profile card after successful upddate", async () => {
    setupInEditMode();
    const input = screen.getByLabelText("Change your username");
    userEvent.clear(input);
  ...
  it("displays confirmation question with cancel and confirm buttons", () => {
    ...
    userEvent.click(deleteButton);
    expect(
      screen.getByText("Are you sure to delete your account?")
    ).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Cancel" })).toBeInTheDocument();
