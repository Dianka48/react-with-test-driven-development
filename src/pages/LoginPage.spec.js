...
describe("Login Page", () => {
  ...
  describe("Interactions", () => {
    ...
    const setup = (email = "user300@mail.com") => {
      render(<LoginPage />);
    ...
    it("sends email and password to backend after clikcing the button", async () => {
      setup();
      userEvent.click(button);
      const spinner = screen.getByRole("status");
      await waitForElementToBeRemoved(spinner);
    ...
    it("displays authentication failed message", async () => {
      setup();
      userEvent.click(button);
    ...
    it("clears aunthentication fail message when email field is changed", async () => {
      setup();
      userEvent.click(button);
      const errorMessage = await screen.findByText("Incorrect credentials");
    ...
    it("clears aunthentication fail message when password field is changed", async () => {
      setup();
      userEvent.click(button);
      const errorMessage = await screen.findByText("Incorrect credentials");
  ...
  describe("Internationalization", () => {
    let slovakToggle, englishToggle;
    const setup = () => {
      render(
        <>
          <LoginPage />
    ...
    it("displays all text in Slovak after changing the language", () => {
      ...
      expect(
        screen.getByRole("heading", { name: sk.login })
      ).toBeInTheDocument();
      expect(
        screen.getByRole("button", { name: sk.login })
      ).toBeInTheDocument();
    ...
    it("sets accept language header to en for outgoing request", async () => {
      ...
      userEvent.type(emailInput, "user300@mail.com");
      userEvent.type(passwordInput, "P4ssword");
    ...
    it("sets accept language header to sk for outgoing request", async () => {
      setup();
      const emailInput = screen.getByLabelText("E-mail");
      const passwordInput = screen.getByLabelText("Password");
      userEvent.type(emailInput, "user300@mail.com");
