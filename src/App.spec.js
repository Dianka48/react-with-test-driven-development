...
describe("Login", () => {
  ...
  it("hides Login and Sign Up from navbar after successful login", async () => {
    ...
    const singUpLink = screen.queryByRole("link", { name: "Sign Up" });
  ...
  it("refreshes user page from another user to the logged in user after clicking My Profile", async () => {
    storage.setItem("auth", { isLoggedIn: true, id: 5, username: "user5" });
    setup("/");
    const userLink = await screen.findByText("user-in-list");
...
describe("Delete user", () => {
  ...
  it("redirects to home page after deleting user", async () => {
